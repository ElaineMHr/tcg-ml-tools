...
-   `mtg_tools/db/initialize_db.py`
-   `mtg_tools/db/populate/populate_decks.py`
-   `mtg_tools/db/populate/populate_deck_stats.py`
-   `mtg_tools/db/populate/populate_cards.py`
-   `mtg_tools/db/populate/populate_keywords_and_types_db.py`

## Cards and Types

Table: `cards` (from `initialize_db.py`)
...
Table: `decks` (from `initialize_db.py`)
...
Table: `formats` (from `initialize_db.py`)
...
Definition (created by `initialize_db.py`; populated by `populate/populate_deck_stats.py`):
...
Cards (`populate/populate_cards.py`)
...
Archetypes (`populate/populate_deck_archetypes.py`)

-   Maps a deck’s weak archetype to multiple archetype tags into `deck_archetypes` with source labels.

Notes (current population status):

-   formats: no population yet.
-   deck_formats: no population yet.
-   archetypes: no population yet; `deck_archetypes` is populated.
-   decks_deduplicated: no population yet.
