...
def parse_type_line(type_line: str, supertypes: set[str], types_dict: set[str]):
    if "—" in type_line:
...
def populate_cards(db_path: str, cards_json_path: str, image_dir: str):
    ...
    skipped_cards: list[tuple[str, str]] = []
    ...
    for card in cards:
        ...
        for face_index, face in enumerate(faces):
            ...
            for t in sups:
                cur.execute(
                    "INSERT INTO card_types (face_id, type_value, type_category) VALUES (?, ?, ?)",
                    (face_id, t, "supertype"),
                )
            for t in typs:
                cur.execute(
            ...
            for kw in face.get("keywords", []):
                if kw in keyword_set:
                    cur.execute(
                        "INSERT INTO card_keywords (face_id, keyword) VALUES (?, ?)",
                        (face_id, kw),
                    )
                else:
